...
def sleep(params_s, params_p, zl, step):
    ...
    #switched to using zl, which is equal to z_waking, because it corresponds to the right step.  
...
if __name__ == "__main__":
    ...
    zn_daydream, xg_daydream, daydream_loss = sleep(params_s, params_p, zl_waking, step)
    #zn_sleep, xg_sleep, sleep_loss = sleep(params_s, params_p, zl_sleep, step)
    ...
    #REVERSE!!
    #disc_sleep, h_sleep = net_d(params_d, zn_sleep*0.1, zl_sleep*0.1, xg_sleep)

    disc_daydream, h_daydream = net_d(params_d, zl_waking*0.1, zn_daydream*0.1, xg_daydream)

    matching_loss = T.abs_(h_waking.mean(axis=0) - h_daydream.mean(axis=0)).sum()

    print "DISC USING sgd 0.0001"

    disc_updates = clip_updates(lasagne.updates.sgd(disc_daydream - disc_waking, params_d.values(), learning_rate=0.0001), params_d.values())
    ...
    gen_updates = lasagne.updates.adam(waking_loss + 0.01 * matching_loss + 0.0 * disc_waking, params_q.values() + params_p.values() + params_zr.values() + params_s.values(), beta1=0.5)
    ...
    gen_train = theano.function([x, zl_waking, step], outputs = [waking_loss, zn_waking, xrec, xg_daydream, matching_loss, zn_daydream], updates = gen_updates)

    disc_train = theano.function([x, zl_waking, step], outputs = [disc_waking, disc_daydream], updates = disc_updates)
    ...
    for i in range(0,85000):
        ...
        for j in range(0,8):
            xl.append(np.random.normal([-0.5,-0.0,-0.0,0,-0.5],[0.01,0.01,0.01,0.01,.01],size=5))
            xl.append(np.random.normal([0.5,0.0,0.0,0,0.5],[0.01,0.01,0.01,0.01,0.01],size=5))
        ...
        for step in range(0,5):
            z_sleep_last = z_sleep
            z_waking_last = z_waking

            waking_loss, z_waking, xrec, xg, matching_loss, z_daydream = gen_train(xseq[:,step:step+1], z_waking_last,step)

            for j in range(5):
                d_waking, d_daydream = disc_train(xseq[:,step:step+1],z_waking_last,step)
            ...
            if i % 100 == 0:
                ...
                print "z daydream min mean max", z_daydream.min(), z_daydream.mean(), z_daydream.max()
                print "d_waking", d_waking
                print "d_sleep", d_daydream
