...
def init_params_s():
    ...
    params['w1'] = theano.shared(0.03 * rng.normal(0,1,size=(1,1024+20,1024)).astype('float32'))
    params['w2'] = theano.shared(0.03 * rng.normal(0,1,size=(1,1024,1024)).astype('float32'))
    params['w3'] = theano.shared(0.03 * rng.normal(0,1,size=(1,1024,1024)).astype('float32'))

    params['b1'] = theano.shared(0.0 * rng.normal(0,1,size=(1,1024,)).astype('float32'))
    params['b2'] = theano.shared(0.0 * rng.normal(0,1,size=(1,1024,)).astype('float32'))
    params['b3'] = theano.shared(0.0 * rng.normal(0,1,size=(1,1024,)).astype('float32'))
...
def net_s(p,z,step):
    ...
    z2 = srng.normal(size=(z.shape[0],20))

    inp = join(z,z2)

    h1 = T.nnet.relu(ln(T.dot(inp, p['w1'][step]) + p['b1'][step]),alpha=0.02)
    h1 *= T.cast(srng.binomial(n=1,p=0.5,size=h1.shape),'float32')
    h2 = T.nnet.relu(ln(T.dot(h1,p['w2'][step]) + p['b2'][step]),alpha=0.02)
    h2 *= T.cast(srng.binomial(n=1,p=0.5,size=h2.shape),'float32')
    h3 = T.dot(h2,p['w3'][step]) + p['b3'][step]
...
if __name__ == "__main__":
    ...
    disc_waking, h_waking = net_d(params_d, zl_waking*0.0, zn_waking*0.0, x)

    #zn_sleep = -2.0 + 0.0*srng.normal(size=zl_sleep.shape) + 0.0*zn_sleep
    
    disc_sleep, h_sleep = net_d(params_d, zl_sleep*0.0, zn_sleep*0.0, xg)

    matching_loss = T.abs_(h_waking.mean(axis=0) - h_sleep.mean(axis=0)).sum()

    disc_updates = clip_updates(lasagne.updates.adam(disc_sleep - disc_waking, params_d.values(), beta1=0.5))
    ...
    gen_updates = lasagne.updates.adam(0.1 * waking_loss + 0.01 * matching_loss + 0.1 * disc_waking - 0.1 * disc_sleep, params_q.values() + params_p.values() + params_zr.values() + params_s.values(), beta1=0.5)
    ...
    for i in range(0,1600):
        ...
        for j in range(0,8):
            xl.append(np.random.normal([0.5,0.5,0,0,1],[0.01,0.01,0.01,0.01,0.1],size=5))
            xl.append(np.random.normal([-0.5,-0.5,0,0,1],[0.01,0.01,0.01,0.01,0.1],size=5))
        ...
        for step in range(0,1):
            waking_loss, z_waking, xrec, z_sleep, xg, matching_loss = gen_train(xseq[:,step:step+1], z_waking, z_sleep,step)

            for j in range(5):
                d_waking, d_sleep = disc_train(xseq[:,step:step+1],z_waking,z_sleep,step)
